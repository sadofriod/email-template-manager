...
interface UseTemplateServiceReturn {
  ...
  getTemplate: (type: ValidTemplateType, id: string, appEntry: ValidAppEntry) => Promise<void>;
  createTemplate: (templateData: CreateTemplateRequest) => Promise<boolean>;
  updateTemplate: (type: ValidTemplateType, id: string, templateData: UpdateTemplateRequest) => Promise<boolean>;
  deleteTemplate: (type: ValidTemplateType, id: string, appEntry: ValidAppEntry) => Promise<boolean>;
  previewTemplate: (type: ValidTemplateType, id: string, appEntry: ValidAppEntry, data: PreviewTemplateRequest) => Promise<{ subject: string, htmlContent: string, textContent: string, from: string } | null>;
  
  // Utility functions
  clearError: () => void;
  clearCurrentTemplate: () => void;
  getTemplateTypeOptions: () => Array<{ value: ValidTemplateType, label: string }>;
...
export function useTemplateService(): UseTemplateServiceReturn {
  ...
  const [currentTemplate, setCurrentTemplate] = useState<TemplateDetail | null>(null);
  ...
  const listTemplates = useCallback(async (type?: ValidTemplateType) => {
    setLoading(true);
    setError(null);
  ...
  const updateTemplate = useCallback(async (
    ...
    try {
      const response = await templateService.updateTemplate(type, id, templateData);
      const data = handleApiResponse(response);
  ...
  ): Promise<{ subject: string, htmlContent: string, textContent: string, from: string } | null> => {
    setLoading(true);
    setError(null);
    
    try {
      const response = await templateService.previewTemplate(type, id, appEntry, previewData);
      const data = handleApiResponse(response);
...
export function useTemplateValidation() {
  const validateTemplateType = useCallback((type: string): type is ValidTemplateType => {
    return templateService.isValidTemplateType(type);
  }, []);

  const validateAppEntry = useCallback((appEntry: string): appEntry is ValidAppEntry => {
    return templateService.isValidAppEntry(appEntry);
  }, []);

  const validateTemplateData = useCallback((data: Partial<CreateTemplateRequest>): string[] => {
    const errors: string[] = [];
